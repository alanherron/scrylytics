...
export async function getCardByName(cardName) {
  if (cardNameCache.has(cardName.toLowerCase())) {
    return cardNameCache.get(cardName.toLowerCase());
  }

  try {
    const cards = await searchCards(`!"${cardName}"`, 1);
    const card = cards[0];

    if (card) {
      cardNameCache.set(cardName.toLowerCase(), card);
    }
